function Vtrans = DispersioVfrontp ()
...
global Ti A DT d0 tol entalpia;
...
Vtrans = zeros(1,5);
VolumMostra = d0*A-tol;
Vtransaux = 0;
%%Inici cálcul%%

while (VolumMostra>Vtransaux)
    T=Ti+i*DT;
    Vtrans(i+1,2)= DispersioDistFrontp(T);%quad2D(@DispersioDistFrontp,100,500,T0,T);
    Vtrans(i+1,1)= T;
